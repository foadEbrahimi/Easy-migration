...
export default function Footer() {
  ...
  const quickLinks = [
    { id: 1, label: tLinks('services'), href: '/' },
    { id: 2, label: tLinks('faq'), href: '/' },
    { id: 3, label: tLinks('blog'), href: '/' },
    { id: 4, label: tLinks('reviews'), href: '/' },
  ];
    ...
    <footer className='bg-primary md:rounded-t-[45px] p-5 pb-4 md:mx-4'>
      ...
      <div className='flex items-center gap-2 justify-center'>
        ...
        <span className='text-2xl font-diba text-white'>
          {t('logo.text1')}{' '}
          <span className='text-[#f6cb32]'>{t('logo.text2')}</span>
        </span>
      </div>
      ...
      <div className='flex flex-col md:flex-row md:justify-between max-w-7xl mx-auto'>
        ...
        <div className='flex md:hidden mt-4 justify-between'>
          <ContactBlock title={tContact('phone')} value='021123456789' />
          <ContactBlock
            title={tContact('email')}
            value='info@easymigration.com'
          />
        </div>
      ...
      <a
        ...
        className='font-medium text-white flex justify-center mt-5 text-body-4 md:text-base hover:text-[#f6cb32] transition-colors duration-300'
      >
        {t('copyright')}
      </a>
    </footer>
